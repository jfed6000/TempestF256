...
*As built (`src/frame.a`, 2026-09-23): **one pass a tick, as Joust**: each pass sleeps to the
next tick, reads the controls and runs 525/128 virtual IRQs (246.09375 Hz exactly), ALHAR2's own,
fed through the input shadows. A game frame is split across passes so that no pass runs past its
tick: a pass for the logic (the clear first, then EXSTAT, NONSTA, DISPLA), then the drawing (the
interpreter into the hidden bitmap, the flip, the CLUT, the text) as a coroutine that gives the
tick back when its budget runs out. `docs/status.md`, "The platform layer".*
...
### 6.3 `$C7 SS.Tick` — WITHDRAWN 2026-09-23

Proposed the same day (a GetStat returning the 60 Hz tick count, so the frame loop could count
the ticks a long pass covers) and withdrawn after review. A call a pass costs about 220 µs, and
the user looked for a way without one. `SOLdrv`'s line signal and `F$CpyMem` of the kernel's
`D.Tick` were weighed and not taken. **What was built instead is Joust's rule, one pass a tick,
with the game frame split across passes** so that no pass runs past its tick (section 2, and
`docs/status.md`, "The platform layer"). No driver change.
